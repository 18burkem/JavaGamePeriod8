...
//	Water
	...
	public Room(File f, int rows, int cols) {
		...
		for(int row = 0; row < rows;row++){
			for(int col = 0; col < cols&&sc.hasNextLine();col++){
				...
				else if(nextLine.matches(WALL))tiles[row][col]= new Tile("Images/"+WALL, true);
				else if(nextLine.matches(DIRT))tiles[row][col]= new Tile("Images/"+DIRT, true);
				else if(nextLine.matches(WATER))tiles[row][col]= new Tile("Images/"+WATER, true);
	...
	public static void main(String[] gusFailedTheFinal){
		Room test = new Room(new File("Files/TestRoom.txt"), 2, 2);
