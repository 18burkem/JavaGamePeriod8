	...
	public Room(File f, int rows, int cols) {
		...
		for(int row = 0; row < rows;row++){
			for(int col = 0; col < cols&&sc.hasNextLine();col++){
				...
				if(nextLine.matches(GRASS))tiles[row][col]= new Tile("resources/Images/"+GRASS+".png", true, null);
				else if(nextLine.matches(WALL))tiles[row][col]= new Tile("resources/Images/"+WALL+".png", false, null);
				else if(nextLine.matches(DIRT))tiles[row][col]= new Tile("resources/Images/"+DIRT+".png", true, null);
				else if(nextLine.matches(WATER))tiles[row][col]= new Tile("resources/Images/"+WATER+".png", false, null);   //row col and all that 
